...
// BOOK FORMATS - Replaced clothing sizes
...
export const ProductDetails = () => {
    ...
    const [selectedFormat, setSelectedFormat] = useState('') // Changed from selectedSize
    ...
    useEffect(() => {
        if (cartItemAddStatus === 'fulfilled') {
            ...
            toast.error('Error adding book to cart, please try again later')
        }
    }, [cartItemAddStatus])
    ...
    useEffect(() => {
        if (wishlistItemAddStatus === 'fulfilled') {
            ...
            toast.error("Error adding book to wishlist, please try again later")
        }
    ...
    useEffect(() => {
        if (wishlistItemDeleteStatus === 'fulfilled') {
            ...
            toast.error("Error removing book from wishlist, please try again later")
        }
    ...
    useEffect(() => {
        if (reviewFetchStatus === 'rejected') {
            toast.error("Error fetching book reviews, please try again later")
        }
    }, [reviewFetchStatus])
    ...
    const handleFormatSelect = (format) => { // Changed from handleSizeSelect
                        ...
                        <Stack>
                            ...
                            <Stack width={is480 ? "auto" : is1420 ? "auto" : '88rem'} p={is480 ? 2 : 0} height={is840 ? "auto" : "50rem"} rowGap={5} mt={is840 ? 0 : 5} justifyContent={'center'} mb={5} flexDirection={is840 ? "column" : "row"} columnGap={is990 ? "2rem" : "5rem"}>
                                    ...
                                    <Stack mt={is480 ? "0rem" : '5rem'}>
                                        {
                                            is1420 ?
                                                <Stack width={is480 ? "100%" : is990 ? '400px' : "500px"}>
                                                    {/* Simple image display - removed SwipeableViews dependency */}
                                                    <div style={{ width: '100%', height: '100%' }}>
                                                        {product?.images && product.images.map((image, index) => (
                                                            <div key={index} style={{ display: index === activeStep ? 'block' : 'none', width: '100%', height: '100%' }}>
                                                ...
                                                :
                                                <div style={{ width: "100%" }}>
                                                    <img style={{ width: "100%", objectFit: "contain", aspectRatio: 1 / 1 }} src={product?.images && product.images[selectedImageIndex]} alt={product?.title} />
                                                </div>
                                        }
                                    </Stack>
                                ...
                                <Stack rowGap={"1.5rem"} width={is480 ? "100%" : '25rem'}>
                                    ...
                                    {/* format and add-to-cart - BOOK SPECIFIC */}
                                    {
                                        !loggedInUser?.isAdmin &&
                                        <Stack sx={{ rowGap: "1.3rem" }} width={'fit-content'}>

                                            {/* Format (book format instead of size) */}
                                                ...
                                                <Stack flexDirection={'row'} columnGap={is387 ? ".5rem" : "1rem"}>
                                                    {
                                                        FORMATS.map((format) => (
                                                            <motion.div
                                                                ...
                                                                whileTap={{ scale: 1 }}
                                                                style={{
                                                                    border: selectedFormat === format ? '' : "1px solid grayText",
                                                                    borderRadius: "8px",
                                                                    width: "30px",
                                                                    height: "30px",
                                                                    display: "flex",
                                                                    ...
                                                                    cursor: "pointer",
                                                                    padding: "1.2rem",
                                                                    backgroundColor: selectedFormat === format ? "#DB4444" : "whitesmoke",
                                                                    color: selectedFormat === format ? "white" : ""
                                                                }}
                                                            >
                                                                <p>{format}</p>
                                                            </motion.div>
